...
- Name: registration.spec.ts >> tc2: forgot pwd link exist test
- Location: tests\registration.spec.ts:25:1

# Error details

```
TypeError: registrationpage.pause is not a function
```
...
```ts
  ...
  13 |   await loginpage.navigateToRegistrationPage();
  ...
  23 | });
  24 | 
  25 | test("tc2: forgot pwd link exist test", async ({}) => {
> 26 |   await registrationpage.pause();
     |                          ^ TypeError: registrationpage.pause is not a function
  27 | });
  28 | 
  29 | test('tc4:user register test', async ({})=>{
