...
test("tc2:login page logo test", async ({loginpage}) => { //remove flower braces as well-Blank destructuring
...
test("tc4:user able to login to the page test", async ({loginpage, homepage}) => {
...
let testrecord = csvhelper.readCsv('src/data/logindata.csv'); //Data Provider
    ...
    for(let row of testrecord){
        ...
        test(`Negative-invalid login test ${row.username} - ${row.password}`, async ({loginpage})=>{
            await loginpage.doLogin(row.username, row.password);
            expect(loginpage.isInvalidLoginMesgeDisplayed()).toBeTruthy();
  ...
  test(`TC_Row_${csvRowNum}: Invalid login test with ${row.username}-${row.password}`, async ({ loginpage }) => {

    await loginpage.doLogin(row.username, row.password);

     expect(
      loginpage.isInvalidLoginMesgeDisplayed()
    ).toBeTruthy();
